...
def command(
    ...
    **attrs: t.Any,
    ...
    """
    Creates a new Command and uses the decorated function as a callback.
    """
...
# Remove the circular dependency on `click.option` by using `option` directly
from .core import Command  # Import only the necessary components from `core`

def decorator(f):
    def new_func(ctx, *args, **kwargs):
        # Execute the wrapped function with the correct context and arguments
        return ctx.invoke(f, *args, **kwargs)
    return new_func

@command()  # Use the `command` decorator directly
@decorator
def my_command():
    print("Hello, world!")

# When `my_command` is executed, `ctx.invoke(f)` ensures it is called correctly.
